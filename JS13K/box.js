...
export default class Box {
	...
	draw(game) {
		...
		let corners = [
			{ x: x, y: y, angle: arcTan(x, y, px, py)},
			{ x: x, y: y + h, angle: arcTan(x, y + h, px, py)},
			{ x: x + w, y: y, angle: arcTan(x + w, y, px, py)},
			{ x: x + w, y: y + h, angle: arcTan(x + w, y + h, px, py)}
		];

		// sort corners by angle clockwise
		corners.sort(function(a, b){

			if(a.angle - b.angle > Math.PI) {
				return -1;
			}
        ...
        context.lineTo(l.x + drawdist * Math.cos(l.angle), l.y + drawdist * Math.sin(l.angle));
        context.lineTo(r.x + drawdist * Math.cos(r.angle), r.y + drawdist * Math.sin(r.angle));
